...
def floyd(_graph, _start, _end):
    ...
    for temp in range(_graph.all_vertex):
        for row in range(_graph.all_vertex):
            for col in range(_graph.all_vertex):
                dist = distance[row][temp] + distance[temp][col]
    ...
    _graph.closed_vertex[_start] = 0
    for i in range(_graph.all_vertex):
        if distance[_start][i] != -1:
            _graph.put_into_closed_vertex(i, distance[_start][i])
...
def main():
    counts = 3
    dist = [[0 for i in range(2)] for i in range(counts)]
    iter_nums = [[0 for i in range(2)] for i in range(counts)]
    ...
    for index in range(counts):
        ...
        dist[index][2] = _dist
        iter_nums[index][2] = len(graph.closed_vertex.keys())
        del graph
